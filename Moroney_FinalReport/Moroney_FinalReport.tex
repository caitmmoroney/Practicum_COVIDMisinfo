...
classifier pipeline, we first turned to LIME (Ribeiro, Singh, and
Guestrin 2016), a popular explainability model which produces local
explanations for classifier decisions. In the context of an NLP
classification task, LIME attempts to provide an accurate explanation
for why the classifier in question made a classification decision for a
given document based on the featurization method used. For this study,
we can apply LIME to obtain an explanation for why the SVM classifier
classified a given tweet as unreliable based on the words which appear
in that tweet; LIME can tell us which class the classifier associates
each word with, as well as the relative importance of each word in the
machine's decision-making process for that tweet.
